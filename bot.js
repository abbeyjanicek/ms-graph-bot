...
const CONNECTION_SETTING_NAME = 'ms-graph-bot';
...
class GraphAuthenticationBot {
    ...
    constructor(conversationState) {
        ...
        this.helpMessage = `You can type "schedule meeting <recipient_email>" to schedule a meeting with someone you know, "me" to view personal information,` +
            ` "users" to see members of your organization, "find meeting times" to find meeting times to help schedule a potential meeting with someone or "help" to view the commands` +
            ` again. Any other text will display your access token to utilize the application.`;
    ...
    async sendWelcomeMessage(turnContext) {
        ...
        if (activity && activity.membersAdded) {
            const heroCard = CardFactory.heroCard(
                'Hi, Im Sonya! I am here to connect you to Microsoft Graph.',
    ...
    async processStep(step) {
        ...
        if (tokenResponse !== undefined) {
            ...
            if (command === 'me') {
                await OAuthHelpers.getMe(step.context, tokenResponse);
            } else if (command === 'schedule') {
                await OAuthHelpers.createEvent(step.context, tokenResponse, parts.split(' ')[1].toLowerCase());
            } else if (command === 'users') {
                await OAuthHelpers.getUsers(step.context, tokenResponse);
            } else {
